	...
	<form method="POST" enctype="multipart/form-data" action="<?php echo BASE_URL; ?>products/edit_action">
		...
		<div class="box">
			...
			<div class="box-body">
				...
				<div class="form-group <?php echo (in_array('id_brand', $errorItems))?'has-error':''; ?>">
					...
					<select required name="id_brand" id="p_brand" class="form-control">
						<?php foreach($brands_list as $item): ?>
							<option <?php echo ($item['id']==$info['id_brands'])?'selected':''; ?> value="<?php echo $item['id']; ?>"><?php echo $item['name']; ?></option>
						<?php endforeach; ?>
					</select>
				</div>
				...
				<div class="form-group <?php echo (in_array('price_sec', $errorItems))?'has-error':''; ?>">
					<label for="p_price_sec">Preço (de)</label>
					<input type="text" class="form-control" id="p_price_sec" name="price_sec" value="<?php echo $info['price_sec']; ?>" />
				</div>
				...
				<div class="form-group <?php echo (in_array('sale', $errorItems))?'has-error':''; ?>">
					...
					<input type="checkbox" id="p_sale" name="sale" <?php echo ($info['Promocao']=='1')?'checked':''; ?> />
				</div>

				<div class="form-group <?php echo (in_array('bestseller', $errorItems))?'has-error':''; ?>">
					<label for="p_bestseller">Mais Vendidos</label><br/>
					<input type="checkbox" id="p_bestseller" name="bestseller" <?php echo ($info['MaisVendido']=='1')?'checked':''; ?> />
				</div>

				<div class="form-group <?php echo (in_array('new_product', $errorItems))?'has-error':''; ?>">
					<label for="p_new_product">Novo Produto</label><br/>
					<input type="checkbox" id="p_new_product" name="new_product" <?php echo ($info['New']=='1')?'checked':''; ?>/>
				</div>
				...
				<div class="images_area">
					<?php foreach($info['images'] as $id_image => $url): ?>
						<div class="p_image">
							...
							<a href="<?php echo BASE_URL.'products/delImg/'.$info['id']; ?> ">[ deletar ]</a>
