...
class ProductsController extends Controller {
	...
	public function add_action() {
		...
		if(!empty($_POST['name'])) {
			...
			$featured = (!empty($_POST['featured']))?1:0;
			$Promocao = (!empty($_POST['sale']))?1:0;
			$MaisVendido = (!empty($_POST['bestseller']))?1:0;
			$New = (!empty($_POST['new_product']))?1:0;
			...
			if(!empty($id_category) && !empty($id_brands) && !empty($Name) && !empty($stock) && !empty($price)) {
				...
				$_SESSION['formError'] = array('id_category', 'id_brands', 'Name', 'stock', 'price');
