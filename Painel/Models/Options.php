...
class Options extends Model {
	...
	public function getAll($check_has_products = false) {
		...
		if($check_has_products) {
			$sql = "SELECT *, (select count(*) from product_options where product_options.id_option = options.id) as product_count FROM options";
		} else {
	...
	public function del($id) {
		...
		$sql = "SELECT count(*) as c FROM product_options WHERE id_option = :id";
		$sql = $this->db->prepare($sql);
		$sql->bindValue(':id', $id);
		$sql->execute();
		$data = $sql->fetch();
