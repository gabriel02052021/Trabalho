...
class Products extends Model {
	...
	public function get($id) {
		...
		if($sql->rowCount() > 0) {
			...
			$sql = "SELECT id_option, p_value FROM product_options WHERE id_product = :id";
			$sql = $this->db->prepare($sql);
			$sql->bindValue(':id', $id);
			$sql->execute();
			...
			$sql = "SELECT id, url FROM product_images WHERE id_product = :id";
			$sql = $this->db->prepare($sql);
			$sql->bindValue(':id', $id);
			$sql->execute();
	...
	public function getAll() {
		...
		$sql = "SELECT id, id_category, id_brands, Name, stock, price, price_sec FROM products";
		...
		if($sql->rowCount() > 0) {
			...
			foreach($array as $key => $item) {
				...
				$brandInfo = $brands->get($item['id_brands']);

				$array[$key]['name_category'] = $catInfo['name'];
				$array[$key]['name'] = $brandInfo['name'];
			}
		}
	...
	public function add($id_category,
		...
		if(!empty($id_category) && !empty($id_brands) && !empty($Name) && !empty($stock) && !empty($price)) {
			...
			$sql = "INSERT INTO products (id_category, id_brands, Name, description, stock, price, price_sec, featured, Promocao, MaisVendido, New, options, weight, width, height, length, diameter) VALUES (:id_category, :id_brands, :Name, :description, :stock, :price, :price_sec, :featured, :Promocao, :MaisVendido, :New, :options, :weight, :width, :height, :length, :diameter)";
			$sql = $this->db->prepare($sql);
			$sql->bindValue(':id_category', $id_category);
			$sql->bindValue(':id_brands', $id_brands);
			$sql->bindValue(':Name', $Name);
			$sql->bindValue(':description', $description);
			$sql->bindValue(':stock', $stock);
			$sql->bindValue(':price', $price);
			$sql->bindValue(':price_sec', $price_sec);
			$sql->bindValue(':featured', $featured);
			$sql->bindValue(':Promocao', $Promocao);
			$sql->bindValue(':MaisVendido', $MaisVendido);
			...
			foreach($options_selected as $optk => $opt) {
				...
				$sql = "INSERT INTO product_options (id_product, id_option, p_value) VALUES (:id_product, :id_option, :p_value)";
				$sql = $this->db->prepare($sql);
	...
	public function edit($id_category,
		...
		if(!empty($id) && !empty($id_category) && !empty($id_brand) && !empty($name) && !empty($stock) && !empty($price)) {
			...
			$sql = "UPDATE products SET id_category = :id_category, id_brands = :id_brand, name = :name, description = :description, stock = :stock, price = :price, price_sec = :price_sec, featured = :featured, Promocao = :sale, MaisVendido = :bestseller, New = :new_product, options = :options, weight = :weight, width = :width, height = :height, length = :length, diameter = :diameter WHERE id = :id";
			$sql = $this->db->prepare($sql);
			$sql->bindValue(':id_category', $id_category);
			...
			$sql = "DELETE FROM product_options WHERE id_product = :id";
			$sql = $this->db->prepare($sql);
			$sql->bindValue(':id', $id);
			$sql->execute();
			...
			if(is_array($c_images)) {
				...
				$sql = "DELETE FROM product_images WHERE id_product = :id AND id NOT IN (".implode(',', $c_images).")";
				$sql = $this->db->prepare($sql);
				$sql->bindValue(':id', $id);
				$sql->execute();
			}			
	...
	public function addProductImage($id, $tmp_name, $type) {
		...
		if(!empty($o_img)) {
			...
			$sql = "INSERT INTO product_images (id_product, url) VALUES (:id_product, :url)";
			$sql = $this->db->prepare($sql);
			$sql->bindValue(':id_product', $id);
	...
	public function del($id) {
		...
		#Excluindo da tabela de produtos
		$sql = "DELETE FROM products WHERE id = :id";
		$sql = $this->db->prepare($sql);
		$sql->bindValue(':id', $id);
		$sql->execute();

		#Excluindo da tabela de opções
		$sql = "DELETE FROM product_options WHERE id_product = :id";
		$sql = $this->db->prepare($sql);
		$sql->bindValue(':id', $id);
		$sql->execute();

		#excluindo da tabela de imagens
		$sql = "DELETE FROM product_images WHERE id_product = :id";
		$sql = $this->db->prepare($sql);
		$sql->bindValue(':id', $id);
		$sql->execute();
	}
